...
```bash
git clone https://github.com/stacknoah/oneandaonly_secondhand.git
cd oneandaonly_secondhand
...
```bash
# 1. DB 초기화 (관리자 계정 생성: admin / admin1234!)
flask --app run.py init-db

# 2. (선택) 데모 데이터 생성 (alice, bob / password1!)
...
기본 관리자 비밀번호는 데모 편의용. 운영 시 환경변수로 교체:

```bash
export ADMIN_PASSWORD='강한비밀번호'   # init-db 전에 설정
export SECRET_KEY='랜덤 고정 키'       # 미설정 시 부팅마다 랜덤 (세션 초기화됨)
```
...
기능·보안 20개 항목 자동 점검 (CSRF, IDOR, XSS, SQLi, 송금 검증, 신고 자동 차단, 로그인 잠금 등).
